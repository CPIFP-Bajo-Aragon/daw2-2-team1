...
<div class="col-10">
    ...
    <input type="text" id="buscador" value="" class="mt-5 mb-5" placeholder="Buscar servicio...">
...
<script>
    ...
    let totalServicios=<?php foreach ($datos['totalServicios'] as $total){
    echo $total;
    }?>;

    let registrosPorPagina=10; 
    let numPaginas=Math.ceil(totalServicios/registrosPorPagina);
    let paginacion=document.getElementById("paginar");
    function paginar(num){
        for(i=0;i<num;i++){
        botonNumPag[i]= document.createElement("button");
        botonNumPag[i].textContent=i+1;
        botonNumPag[i].setAttribute("id", i+1);
        paginacion.appendChild(botonNumPag[i]);
        }   
    }
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function actualizarFiltrosDesdePagina(idBoton) {
        ...
        offset=((idBoton-1)*registrosPorPagina);
        console.log(offset);


        filtrar(selectedMunicipioId, tipoServicio, busqueda, offset);
    }

    function actualizarFiltros(idBoton) {
        selectedMunicipioId = selectMunicipio.value;
    ...
    // Escuchar cambios en el select y filtrar automáticamente
        ...
        botonNumPag.forEach(element => {
            
            element.addEventListener("click", function(event) {
                actualizarFiltrosDesdePagina(event.target.id);
            });
    ...
    paginar(numPaginas);
    filtrar(idMunicipio, tipoServicio, buscador, offset);

    function filtrar(selectedMunicipioId, tipoServicio, buscador, offset){
        console.log(selectedMunicipioId);
        console.log(tipoServicio);
        //console.log(offset);
        console.log(buscador);

        fetch('/ServiciosControlador/obtenerServicios/'+selectedMunicipioId+'/'+tipoServicio+'/'+buscador+'/'+offset)
        .then(response => {
            if(!response.ok){
